...
#horProgressToastHost{position:fixed;right:14px;top:14px;z-index:2147483000;pointer-events:none;max-width:min(390px,calc(100vw - 28px));font-family:system-ui,-apple-system,Segoe UI,sans-serif}
...
#horProgressPreview{position:fixed;inset:3vh 3vw;z-index:2147482500;background:#0d120ff7;color:#fff;border:2px solid #b88a31;border-radius:18px;box-shadow:0 20px 70px #000;display:none;overflow:auto;padding:18px;font-family:system-ui,-apple-system,Segoe UI,sans-serif}#horProgressPreview.show{display:block}#horProgressPreview button{background:#2b342b;color:#fff;border:1px solid #b88a31;border-radius:9px;padding:9px 12px;margin:4px;font-weight:700}#horProgressPreview table{width:100%;border-collapse:collapse;margin-top:12px}#horProgressPreview th,#horProgressPreview td{padding:7px;border-bottom:1px solid #ffffff18;text-align:left}#horProgressPreview .warn{color:#ffd276}#horProgressPreview .ok{color:#aee6a5}`;document.head.appendChild(st);}
...
function showPreview(){ensureCss();let el=document.getElementById('horProgressPreview');if(!el){el=document.createElement('div');el.id='horProgressPreview';document.body.appendChild(el);}const personal=findPersonal(),all=simulateAll();let body='<div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><h2 style="margin:0">Progression Dry Run</h2><div class="ok">READ ONLY — nothing on this screen awards, migrates, uploads, or changes statistics.</div></div><button onclick="HORProgression.preview.hide()">Close</button></div>';
...
body+='<p style="opacity:.75;margin-top:18px">Migration marker currently: <b>'+(localStorage.getItem(MIGRATION_KEY)||'NOT MIGRATED')+'</b>. This preview does not change it.</p>';el.innerHTML=body;el.classList.add('show');return {personal,all};}
...
P.preview={legacyStore,normalizeLegacy,simulateName,simulateAll,findPersonal,dryRunDelta,show:showPreview,hide:hidePreview,celebrate:previewCelebration};
})();
